...
asciidoctor  -I /share/Git/asciidoctor-rouge/lib -r asciidoctor-rouge.rb -a rouge-css=external -a rouge-theme=../styles/rouge_github_simonis.css -a linkcss -a stylesheet=github_simonis.css -a stylesdir=./styles -a copycss=./styles/github_simonis.css --destination-dir github_simonis --out-file=index.html -r ./ruby/callout-macro.rb --trace LinuxProcessLayout.adoc
...
[source, highlight="3,5", options="nowrap"]
----
...
As demonstrated in <<Hello_world_pmap>>, we will use the http://man7.org/linux/man-pages/man1/pmap.1.html[`pmap`] utility callout:1[] to display the memory layout of a process. `pmap` is actually just a front-end for the memory map of a process as recorded in the `smaps` file of the http://man7.org/linux/man-pages/man5/proc.5.html[`pmap proc file system`].

.The memory layout of our "Hello world" example
[[Hello_world_pmap]]
[source, console, options="nowrap"]
----
simonis@simonis:/tmp$ pmap -X `pidof hello`
